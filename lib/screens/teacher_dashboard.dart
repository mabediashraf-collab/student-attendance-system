...
import 'teacher_marks_view.dart';
...
class _TeacherDashboardState extends State<TeacherDashboard> {
    ...
    return Scaffold(
      ...
      body: isLoading
          ...
          : _currentIndex == 0
              ...
              : _currentIndex == 1
                  ? _buildMyClassesTab()
                  : _currentIndex == 2
                      ? TeacherAttendanceView(teacherClasses: teacherClasses)
                      : _currentIndex == 3
                          ? const EnterMarksScreen()
                          : _currentIndex == 4
                              ? TeacherAttendanceView(teacherClasses: teacherClasses)
                              : TeacherMarksView(teacherClasses: teacherClasses),
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: const [
          ...
          BottomNavigationBarItem(
              icon: Icon(Icons.edit_calendar), label: 'Take Attendance'),
          BottomNavigationBarItem(
              icon: Icon(Icons.edit_note), label: 'Enter Marks'),
          BottomNavigationBarItem(
              icon: Icon(Icons.history), label: 'Attendance Records'),
          BottomNavigationBarItem(
              icon: Icon(Icons.history), label: 'Marks Records'),
        ],
      ),
    );
  }
  ...
  Widget _buildProfileTab() {
    ...
    String staffNo = teacherData!['staff_no'] ?? '';
    String phone = teacherData!['phone'] ?? '';
    String address = teacherData!['address'] ?? '';
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildInfoRow(Icons.badge, 'Staff Number',
                      staffNo.isNotEmpty ? staffNo : 'Not assigned'),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: ListTile(
            title: Text('${classItem['class_name']} ${classItem['stream']}'),
            subtitle: Text('Students: ${classItem['student_count']}'),
